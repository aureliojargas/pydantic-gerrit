...
models-implemented:
	@grep '^class.*BaseModel' src/pydantic_gerrit/$(version_package)/$(endpoint).py | cut -d ' ' -f 2 | cut -d '(' -f 1 | sort

# Conditions for a model to be considered tested:
# - .model_valite() has called
...
models-not-tested:
	@$(MAKE) --quiet models-implemented models-tested | sort | uniq -u
